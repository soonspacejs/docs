...
### 定义：
```ts
function flyMainViewpoint(viewpoint: TlyToViewpoint = 'frontTop', options: FlyToObjOptions = {}): Promise<[AnimationReturn<Position>, AnimationReturn<Rotation>]>
```
### 用法：
```js
ssp.flyMainViewpoint('top')
...
function flyToObj(object: BaseObject3D | BaseMesh, viewpoint: TlyToViewpoint = 'frontTop', options: FlyToObjOptions = {}): Promise<[AnimationReturn<Position>, AnimationReturn<Rotation>]>
```
### 用法：
```js
ssp.flyToObj(
...
<Docs-Table
    :data="[
      ...
      {
        prop: 'duration', desc: '循环', type: 'number | boolean', require: false, default: 'false'
      }
    ]"
/>
...
### 定义：
```ts
function flyTo(position: Position, rotation: TlyToViewpoint | Rotation = 'frontTop', options?: AnimationOptions): Promise<[AnimationReturn<Position>, AnimationReturn<Rotation>]>
```
### 用法：
```js
ssp.flyTo(
...
### 定义：
```ts
function surroundOnTarget(position: Position, rotation: TlyToViewpoint | Rotation = 'frontTop', options?: AnimationOptions): Promise<[AnimationReturn<Position>, AnimationReturn<Rotation>]>
```
### 用法：
```js
ssp.surroundOnTarget(
